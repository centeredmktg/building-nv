...
export default function ChangeOrderSigningBlock({
  token,
  executed,
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch(`/api/sign/change-order/${token}`, {
      method: 'POST',
  ...
  if (status === 'done') {
    return (
      <div className="border border-green-800 bg-green-950/30 rounded-sm p-6">
        <p className="text-green-400 font-semibold mb-1">Change Order Signed</p>
  ...
  return (
    <div className="border border-gray-300 rounded-sm p-6 print:hidden">
      <h3 className="text-gray-900 font-semibold mb-1">Sign Change Order</h3>
      <p className="text-gray-600 text-sm mb-4">
        By signing below and clicking Submit, you agree to the terms of this change order and authorize Building NV to proceed with the work described herein.
      </p>
      ...
      >
        {status === 'loading' ? 'Submitting\u2026' : 'Submit Signed Change Order'}
      </button>
    </div>
  );
}
