...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: `${quote.address} | ${client.name} | Quote Summary` };
}

const fmt = (n: number) =>
  n.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });

export default async function ProposalSummaryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const quote = await getQuoteBySlug(slug);
  ...
  // Section subtotals
  const sectionTotals = quote.sections.map((sec) => ({
    title: sec.title,
    subtotal: sec.items.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0),
  }));
      ...
      <div className="max-w-3xl mx-auto px-8 print:px-6">
        ...
        <div className="mb-10 print:mb-6">
          <div className="bg-white rounded border border-[#E8E4DD] overflow-hidden">
            {sectionTotals.map((sec, i) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#1E2A38] text-white text-[10px] font-bold shrink-0">
                    {i + 1}
                  </span>
                  <span className="text-[#3A3530] font-medium">{sec.title}</span>
                </div>
            ...
            {/* Subtotal row */}
            <div className="border-t border-[#DDD8D0] px-5 py-3 flex justify-between bg-[#F7F5F0]">
              <span className="text-[#6B6560] text-sm font-medium">Subtotal</span>
              <span className="text-[#1A1917] text-sm font-semibold tabular-nums">${fmt(totals.subtotal)}</span>
            </div>
          </div>
        </div>
