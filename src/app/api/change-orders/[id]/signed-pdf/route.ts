...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const changeOrder = await prisma.changeOrder.findUnique({ where: { id }, select: { signedPdfPath: true } });
  if (!changeOrder?.signedPdfPath) return new NextResponse('Not found', { status: 404 });

  const resolvedPath = path.resolve(changeOrder.signedPdfPath);
